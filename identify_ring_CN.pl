...
for($i=0;$i<@chainID;$i++)
{for($k=0;$k< @{$chainID[$i]}/3;$k++)
  {@ring=();
   $x_1[$k]=substr(${$chainID[$i]}[$k*3],30,8);
   $y_1[$k]=substr(${$chainID[$i]}[$k*3],38,8);
   $z_1[$k]=substr(${$chainID[$i]}[$k*3],46,8);
   $x_c[$k]=substr(${$chainID[$i]}[$k*3+1],30,8);
   $y_c[$k]=substr(${$chainID[$i]}[$k*3+1],38,8);
   $z_c[$k]=substr(${$chainID[$i]}[$k*3+1],46,8);
   $x_n[$k]=substr(${$chainID[$i]}[$k*3+2],30,8);
   $y_n[$k]=substr(${$chainID[$i]}[$k*3+2],38,8);
   $z_n[$k]=substr(${$chainID[$i]}[$k*3+2],46,8);
   $vec_1x=0;
   $vec_1y=0;
   $vec_1z=0;
   $vec_1x=$x_1[$k]-$x_c[$k];
   $vec_1y=$y_1[$k]-$y_c[$k];
   $vec_1z=$z_1[$k]-$z_c[$k];##vector from C of CN to attached atom 
   $mm=0;
   foreach $line(@lig)
     {if((substr($line,21,1) eq $chainID[$i])&&(substr($line,77,1) ne "H")&&(substr($line,77,1) ne "F")&&(substr($line,76,2) ne "BR")&&(substr($line,76,2) ne "CL")&&(substr($line,76,2) ne " I")&&(substr($line,30,8) != $x_1[$k])&&(substr($line,30,8) != $x_c[$k])&&(substr($line,30,8) != $x_n[$k]))
       {$ligand[$mm]=$line;
        $mm++;
       }
     }
   ...
   foreach $line(@ligand)
     {
      ...
      $distX=sqrt(($x_1[$k]-$x_x)**2+($y_1[$k]-$y_x)**2+($z_1[$k]-$z_x)**2);
   ...
   if($bond==1)
     ...
     {$x5=substr($atom[0],30,8);
      ...
      $vec_5x=$x5-$x_1[$k];
      $vec_5y=$y5-$y_1[$k];
      $vec_5z=$z5-$z_1[$k];#vector from CN attached atom to atom5
      $vec_6x=$x6-$x_1[$k];
      $vec_6y=$y6-$y_1[$k];
      $vec_6z=$z6-$z_1[$k];#vector from CN attached atom to atom6
      ...
      $vec_2x=$mid_x-$x_1[$k];
      $vec_2y=$mid_y-$y_1[$k];
      $vec_2z=$mid_z-$z_1[$k];#vector from attached atom to middle point 
      ...
      if(($cos_3<-0.173)&&($cos_3>0.175)) ##the angle between $vec_1 and $nomal is less than 80,more than 100
         ...
         if(($aa==1)&&($bb==1))
          {
           ...
           if(($dist_7<1.80)&&($dist_7>1.20)&&($cos_5>-0.173)&&($cos_5<0.175)&&($cos_6>-0.173)&&($cos_6<0.175)) #the angle more than 80less than 100,the distance is double bond
             {#print "The $k CN is Attached to a five plane ring in chain $chainID[$i] 111\n";
               print "$ARGV[0]\n";
              $ring[0]=${$chainID[$i]}[$k*3];
              $ring[1]=$atom[0];
              $ring[2]=$atom[1];
              $ring[3]=$atom5[0];
              $ring[4]=$atom6[0];
             ...
             {foreach $line_3(@ligand)
                {$x_z=substr($line_3,30,8);
                 ...
                 if(($dist_8<1.80)&&($dist_8>1.20)&&($dist_9<1.80)&&($dist_9>1.20))
                   {#print "The $k CN is Attached to a six plane ring in chain $chainID[$i] 111\n";
                    print "$ARGV[0]\n";
                    $ring[0]=${$chainID[$i]}[$k*3];
                    $ring[1]=$atom[0];
                    $ring[2]=$atom[1];
               ...
               foreach $line_3(@ligand)
                {$x_z=substr($line_3,30,8);
                 ...
                 if(($tmp[0]>2.25)&&($tmp[0]<2.62)&&($dist_9>1.2)&&($dist_9<1.80)&&($dist_10>1.2)&&($dist_10<1.80)&&($cos_4>0.94)&&($cos_4<1)&&($cos_5>-0.173)&&($cos_5<0.175)&&($cos_6>-0.173)&&($cos_6<0.175))
                   {#print "The $k CN is Attached to a six plane ring in chain $chainID[$i] 222 \n";
                     print "$ARGV[0]\n";
        ...
        if(($aa==2)&&($bb==2))
          {
              ...
              foreach $line_3(@ligand)
                {$x_z=substr($line_3,30,8);
                 ...
                 if(($tmp[0]>2.25)&&($tmp[0]<2.62)&&($dist_11>1.2)&&($dist_11<1.80)&&($dist_12>1.2)&&($dist_12<1.80)&&($cos_4>0.94)&&($cos_4<1)&&($cos_5>-0.173)&&($cos_5<0.175)&&($cos_6>-0.173)&&($cos_6<0.175))
                   {#print "The $k CN is Attached to a six plane ring in chain $chainID[$i] 333 \n";
                     print "$ARGV[0]\n";
                    $ring[0]=${$chainID[$i]}[$k*3];
                    $ring[1]=$atom[0];
                    $ring[2]=$atom[1];
           ...
           if(($dist_7<1.80)&&($dist_7>1.20)&&($cos_5>-0.173)&&($cos_5<0.175)&&($cos_6>-0.173)&&($cos_6<0.175))
             {#print "The $k CN is Attached to a five plane ring in chain $chainID[$i] 333 \n";
              print "$ARGV[0]\n";
              $ring[0]=${$chainID[$i]}[$k*3];
              $ring[1]=$atom[0];
              $ring[2]=$atom[1];
        ...
        if(($aa==2)&&($bb==1))
          {@tmp=();
             ...
             {$cos_4=($vec_3x*$vec_5x+$vec_3y*$vec_5y+$vec_3z*$vec_5z)/(sqrt($vec_3x**2+$vec_3y**2+$vec_3z**2)*sqrt($vec_5x**2+$vec_5y**2+$vec_5z**2));
              ...
              foreach $line_3(@ligand)
                {$x_z=substr($line_3,30,8);
                 ...
                 if(($tmp[0]>2.25)&&($tmp[0]<2.62)&&($dist_9>1.2)&&($dist_9<1.80)&&($dist_10>1.2)&&($dist_10<1.80)&&($cos_4>0.94)&&($cos_4<1)&&($cos_5>-0.173)&&($cos_5<0.175)&&($cos_6>-0.173)&&($cos_6<0.175))
                   {#print "The $k CN is Attached to a six plane ring in chain $chainID[$i] 444 \n";
                    print "$ARGV[0]\n";
                    $ring[0]=${$chainID[$i]}[$k*3];
                    $ring[1]=$atom[0];
                    $ring[2]=$atom[1];
           ...
           if(($dist_7<1.80)&&($dist_7>1.20)&&($cos_5>-0.173)&&($cos_5<0.175)&&($cos_6>-0.173)&&($cos_6<0.175))
             {#print "The $k CN is Attached to a five plane ring in chain $chainID[$i] 444 \n";
              print "$ARGV[0]\n";
              $ring[0]=${$chainID[$i]}[$k*3];
              $ring[1]=$atom[0];
              $ring[2]=$atom[1];
      ...
      if($res eq $res_now)
      ...
      else
        {
         ...
         {foreach $line(@array)
          ...
          $v_nnx=$p_x-$r_x;
          $v_nny=$p_y-$r_y;
          $v_nnz=$p_z-$r_z;##vector from center of ring to center of residue 
          $cos_xy=($nomal_rx*$v_nnx+$nomal_ry*$v_nny+$nomal_rz*$v_nnz)/(sqrt($nomal_rx**2+$nomal_ry**2+$nomal_rz**2)*sqrt($v_nnx**2+$v_nny**2+$v_nnz**2));
          $cos_xy=abs($cos_xy);
          $sin_xy=sqrt(1-$cos_xy**2);
          $r1=$cos_xy*$dist_pi;#height
          $r2=$sin_xy*$dist_pi;#width
          ...
          if(($r1<4.5)&&($r2<1.2)&&((($cos_pi>0.93)&&($cos_pi<1))||(($cos_pi>-1)&&($cos_pi<-0.93))))
            {#print "$ARGV[0]:The ring sandwich pi interaction with $res_now in chain $chainID[$i] with distance $r1 and $r2 and angle $angle \n";  ###sprintf("%.2f", $number)
            }
          #if(($res_now=~/^TRP/)&&($r1<4.5)&&($r2<2)&&((($cos_pi>0.93)&&($cos_pi<1))||(($cos_pi>-1)&&($cos_pi<-0.93))))
          #  {print "The ring sandwich pi interaction with $res_now in chain $chainID[$i] with distance $r1 and $r2 and angle $angle \n";
          #  }
          if(($r1<4.5)&&($r2<2.4)&&($r2>1.2)&&((($cos_pi>0.93)&&($cos_pi<1))||(($cos_pi>-1)&&($cos_pi<-0.93))))
            {#print "$ARGV[0]:The ring parallel displaced pi interaction with $res_now in chain $chainID[$i] with distance $r1 and $r2 and angle $angle \n";
            }
          #if(($res_now=~/^TRP/)&&($r1<4.5)&&($r2<3.4)&&($r2>2)&&((($cos_pi>0.93)&&($cos_pi<1))||(($cos_pi>-1)&&($cos_pi<-0.93))))
          #  {print "The ring parallel displaced pi interaction with $res_now in chain $chainID[$i] with distance $r1 and $r2 and angle $angle \n";
          #  }
          if(($dist_pi<5.5)&&(($cos_pi>-0.34)&&($cos_pi<0.34)))
            {#print "$ARGV[0]:The ring edge to face pi interaction with $res_now in chain $chainID[$i] with distance $dist_pi and angle $angle \n";
            }
          }
          $res_now=$res;
          @array=();
